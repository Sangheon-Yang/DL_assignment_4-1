'''11week Yang-Sangheon'''

import tensorflow as tf
import numpy as np

# S: 디코딩 입력의 시작을 나타내는 심볼
# E: 디코징 출력의 끝을 나타내는 심볼
...
def make_batch(seq_data):
    ...
    for seq in seq_data:
        #인코더 셀의 입력값, 입력단어의 글자들을 한글자씩 뗴어 배열로 만든다.
        input = [num_dic[n] for n in seq[0]]
        #디코더 셀의 입력값, 시작을 나타내는 S심볼을 맨앞에 붙여준다.
        output = [num_dic[n] for n in ('S' + seq[1])]
        # 학습을 위해 비교할 디코더 셀의 출력값, 끝나ㄴㄴ 것을 알려주기 위해 마지막에 E를 붙인다.
...
#--------------- attention --------------#
# alpha dimmension : matmul ((B,S1,H), (B,H,S2)) = (B,S1,S2)
alphas =  tf.nn.softmax(  tf.matmul(enc_outputs, tf.transpose( dec_outputs ,  perm = [0,2,1]))  )
#print(alphas)
# attention dimmention: (B, S2 S1) * (B,S1,H) = (B, S2, H)
...
##번역 테스트
#단어를 입력받아 번역단어를 예측하고 디코딩 하는 함수
def translate(word):
    #이 모델은 입력값과 출력값 데이터로 [영어단어, 한글단어]를 사용하지만,
    #예측시에는 한글단어를 알지 못하므로, 디토더의 입출력갑을 의미없는 값인 P로 채운다.
    #['word', 'PPPP']

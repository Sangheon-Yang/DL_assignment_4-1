...
W3 = tf.get_variable(name="W3", shape = [3, 3, 64, 64], initializer = tf.contrib.layers.xavier_initializer())
b3 = tf.get_variable(name="b3", shape = [64], initializer = tf.contrib.layers.xavier_initializer())
C3 = tf.nn.conv2d(L2_pool, W3, strides=[1, 1, 1, 1], padding = 'SAME')
L3 = tf.nn.relu(tf.nn.bias_add(C3, b3))
#L2_pool = tf.nn.max_pool(L2, ksize=[1, 2, 2, 1], strides= [1, 2, 2, 1], padding='SAME')
L3_flat = tf.reshape(L3, [-1, 64*7*7])

W4 = tf.get_variable(name="W4", shape = [64*7*7, 10], initializer = tf.contrib.layers.xavier_initializer())
b4 = tf.get_variable(name="b4", shape = [10], initializer = tf.contrib.layers.xavier_initializer())
logits = tf.nn.bias_add(tf.matmul(L3_flat, W4), b4)
